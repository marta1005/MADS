...
from scenario import MADSScenario, RandomVariable, Variable
...
##################### Modified #####################
fuselage_length = RandomVariable(
    "fuselage_length",
    3.037,
    sigma=0.005,
...
fuel_used_total = Variable(
    "fuel_used_total",
    100.0,
    output_type="fuel_used_total",
)
...
k = dataset.get_view(variable_names=[fuselage_length.name]).values.flatten()
j = dataset.get_view(variable_names=[range.name]).values.flatten()
...
# plot hybridization_hover histogram
counts, bins = np.histogram(x)
plt.figure()
plt.stairs(counts, bins)
plt.xlabel("hybridization_hover")
plt.ylabel("count")

# plot hybridization_cruise histogram
counts, bins = np.histogram(y)
plt.figure()
plt.stairs(counts, bins)
plt.xlabel("hybridization_cruise")
plt.ylabel("count")

# plot fuselage_length histogram
counts, bins = np.histogram(k)
plt.figure()
plt.stairs(counts, bins)
plt.xlabel("fuselage_length")
plt.ylabel("count")

# plot range histogram
counts, bins = np.histogram(j)
plt.figure()
plt.stairs(counts, bins)
plt.xlabel("range")
plt.ylabel("count")

# plot m_tom histogram
