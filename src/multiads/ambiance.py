...
# https://github.com/airinnova/ambiance/blob/232032bbac7a8ff12e8bf9e888f6179d9094b0e9/src/ambiance/ambiance.py
...
class CONST:
    ...
    LAYER_SPEC_PROP = [
        [-5.0e3, 320.65, -6.5e-3, 1.77687e5, _LAYER_NAME_a],
        [0.00e3, 288.15, -6.5e-3, 1.01325e5, _LAYER_NAME_a],
        [11.0e3, 216.65, 0.0e-3, 2.26320e4, _LAYER_NAME_b],
        [20.0e3, 216.65, 1.0e-3, 5.47487e3, _LAYER_NAME_c],
        [32.0e3, 228.65, 2.8e-3, 8.68014e2, _LAYER_NAME_c],
        [47.0e3, 270.65, 0.0e-3, 1.10906e2, _LAYER_NAME_d],
        [51.0e3, 270.65, -2.8e-3, 6.69384e1, _LAYER_NAME_e],
        [71.0e3, 214.65, -2.0e-3, 3.95639e0, _LAYER_NAME_e],
...
class Atmosphere:
    ...
    * Single values, vectors or matrices are accepted as input
    ...
    def __init__(self, h, check_bounds=True):
        ...
        if check_bounds and (
            (self.h < CONST.h_min - _EPS).any() or (self.h > CONST.h_max + _EPS).any()
        ):
            raise ValueError(
                "Value out of bounds."
    ...
    def _get_layer_nums(self):
        ...
        for i in layers.keys():
            pos_in_layer = (self.H >= layers[i]["H_base"]) & (
                self.H < layers[i]["H_top"]
            )
    ...
    def pressure(self):
        ...
        pressue_beta_nozero = p_b * (1 + (beta / T_b) * (self.H - H_b)) ** (
            exponent * (-CONST.g_0 / CONST.R)
        )
