...
from assembly import Propeller as Base_Propeller

# try import of api
try:
    from xrotor.xrotor import XRotor
    from xrotor.model import Case
except ValueError:
    print("xrotor python api libs has yet to be correctly linked to MADS")
...
@dataclass
class Options:
    thrust_target: float=0.0 # target thrust
    free: bool = True   # free propeller
    duct: bool = False  # ducted propeller
    wind: bool = False  # wind-mill
    altitude: float=1.0 # altitude
    print_file: bool = False # I/O to file

@dataclass
class Propeller:
    r_tip: float       # radius blade tip
    pitch: float       # pitch
    n_blades: float    # number of blades
    rpm: float         # rpm
    r_hub: float=0.3            # radius hub
    rake: float=0.0             # rake angle
    geomdata : np.ndarray=0.0   # Array with the geometric data
    case : dict = field(default_factory=dict)  # nested dictionary with xrotor case info
    r_wake: float=0.0      # Wake blockage radius in m
    power: float=1.0       # prop power
    thrust: float=0.0       # prop thrust
    adv: float=0.1          # advance ratio

    @classmethod
    def from_component(cls, comp: Base_Propeller):
        propeller = Propeller(
            r_tip=comp.r_tip,
        ...
        if "options" in vars(comp):
            for k, v in comp.options.get("prop_xrotor", {}).items():
                if k in vars(propeller):
...
class Driver:
    def __init__(
        ...
        self.soundSpeed = environment.sound
        self.viscosity = environment.viscosity
    ...
    def run(self):
        ...
        # instatiate case
        case_xr = self.propeller.case
        case_xr['conditions']['al'] = self.options.altitude
        case_xr['conditions']['rho'] = self.density
        case_xr['conditions']['rmu'] = self.viscosity
        case_xr['conditions']['vel'] = self.velocity
        case_xr['conditions']['adv'] = self.propeller.adv

        case_xr['disk']['n_blds'] = self.propeller.n_blades
        case_xr['disk']['blade']['radii'] = self.propeller.geomdata[:, 0]
        case_xr['disk']['blade']['chord'] = self.propeller.geomdata[:, 1]
        case_xr['disk']['blade']['twist'] = self.propeller.geomdata[:, 3]
        # assign case to solve
        self.xr.case = Case.from_dict(self.propeller.case)

        # update case information
        self.xr.r_hub = self.propeller.r_hub
        self.xr.r_tip = self.propeller.r_tip
        self.xr.r_wake = self.propeller.r_wake
        self.xr.rake = self.propeller.rake
        self.xr.geomdata = self.propeller.geomdata

        # compute perfromance
        self.xr.operate(rpm=self.propeller.rpm,thrust=self.propeller.thrust)
        perf = self.xr.performance

        self.thrust = perf.thrust
        self.efficiency = perf.efficiency
        self.shaft_power = perf.power
        # if you want to print performance
        if self.options.print_file==True:
            self.print_perf(perf)
    ...
    def shaft_power(self):
        return self.shaft_power

    def print_perf(self,perf):
        file = open('xrotor.log','a')
